...
//Get Records
$newarayforsearch = array();
foreach($column_names as $column_sewarchquery){
		$newarayforsearch[$column_sewarchquery]  = $_SESSION['query'];
}
if($perpage  == "all"){
	$get_records = $database->select($table_name, $column_names,array("LIKE" => array("OR"=>$newarayforsearch)));
}
else{
	$get_records = $database->select($table_name, $column_names, array("LIKE" => array("OR"=>$newarayforsearch)));
}
$count_records = $database->count($table_name, array("LIKE" => array("OR"=>$newarayforsearch)));  //Count Records
if($perpage == "all"){
...
$tpl->current_records = count($get_records);
